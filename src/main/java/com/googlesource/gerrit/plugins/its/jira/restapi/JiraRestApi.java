...
public class JiraRestApi<T> {
   ...
   * @param failCode HTTP response codes allowed and not fail on unexcepted response
   * @throws IOException generated if unexpected failCode is returned
   */
  public T doGet(String spec, int passCode, int[] failCodes, Class<T> classOfT) throws IOException {
  ...
  public T doGet(String spec, int passCode, Class<T> classOfT) throws IOException {
    return doGet(spec, passCode, null, classOfT);
  }
   ...
   * Object of type 'T' is returned containing the parsed JSON data
   */
  public T doPost(String spec, String jsonInput, int passCode, Class<T> classOfT) throws IOException {
    HttpURLConnection conn = null;
    try {
      conn = prepHttpConnection(spec, true);
      writePostData(jsonInput, conn);
      if (validateResponse(conn, passCode, null)) {
        readIncomingData(conn, classOfT);
      }
      return data;
    } finally {
   ...
   * returns true if valid response is returned, otherwise false
   */

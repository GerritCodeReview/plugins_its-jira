...
import static com.googlesource.gerrit.plugins.its.jira.JiraItsServerInfo.Builder.ERROR_MSG;
...
public class JiraItsServer implements ItsFacadeFactory {
  ...
  private boolean setConfigValues(PluginConfig config) {
    try {
      projectUrl =
          adjustUrlPath(
              new URL(jiraConfig.getFromProjectConfig(config, JiraConfig.PROJECT_CONFIG_URL_KEY)));
      projectUsername =
  ...
  private void setValuesUsingGerritConfig() {
    try {
      projectUrl =
          adjustUrlPath(new URL(jiraConfig.getFromGerritConfig(JiraConfig.GERRIT_CONFIG_URL)));
